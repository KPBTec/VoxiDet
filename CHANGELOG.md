...
- Corregido: otro permiso más en la misma cadena impedía que el firewall dinámico terminara de
  aplicarse. Corregido y verificado en servidor real. Aplicar corriendo `deploy.sh` de nuevo.

---

## v1.28.8 — 2026-09-21

- Corregido: el firewall dinámico del panel no lograba aplicarse en instalaciones sin reglas
  configuradas todavía. Corregido y verificado. Aplicar corriendo `deploy.sh` de nuevo.
...
- Corregido: el ajuste de memoria de la versión anterior (v1.28.3) no tenía efecto real una vez
  probado a fondo — reemplazado por uno verificado (cambio del gestor de memoria interno del proceso).
  Aplicar corriendo `deploy.sh` de nuevo.
...
- Corregido: en la primera prueba real del modo "Audiosocket" contra una central telefónica en
  producción, se encontró que la central rechazaba la conexión por un detalle de formato en el
  identificador de llamada. Corregido — actualizá el dialplan de cualquier cliente que ya hayas migrado
  a Audiosocket con la línea nueva que te muestra el panel (Clientes → Editar → sección Audiosocket).
