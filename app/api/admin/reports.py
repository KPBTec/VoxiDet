...
async def reports_page(
    ...
    if mode == "month":
        ...
        prev_total_row = _sum_rows_month(await query_report_month(prev_month, client_id))
    else:
        ...
        prev_total_row = _sum_rows_day(await query_report_day(prev_day, client_id))
...
async def reports_export_csv(
    ...
    if mode == "month":
        month_str = month or today.strftime("%Y-%m")
        rows = await query_report_month(month_str, client_id_int)
        ...
        rows = await query_report_day(day_str, client_id_int)
